...
const SHOP_ADDRESS = '17 Bree Street, Cape Town, 8001';
const SHOP_PHONE = '+27 21 555 0147';
const SHOP_PHONE_HREF = 'tel:+27215550147';
const SHOP_EMAIL = 'hello@crownandblade.co.za';
const SHOP_WHATSAPP = 'https://wa.me/27215550147';
const SHOP_DIRECTIONS = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(SHOP_ADDRESS)}`;
...
const stockBarbers: Barber[] = [
  { id: 1, name: 'Mandla Ndlovu', specialty: 'Fades & texture', bio: 'Mandla works in clean lines and soft transitions, bringing a measured eye to every fade.', imageUrl: 'https://images.unsplash.com/photo-1653758265969-b048bb0b328a?auto=format&fit=crop&w=900&q=85' },
  { id: 2, name: 'Lena Jacobs', specialty: 'Classic cuts & beard work', bio: 'Lena favours timeless shapes with just enough edge, from a close crop to a sculpted beard.', imageUrl: 'https://images.unsplash.com/photo-1643252824297-1670be2a973a?auto=format&fit=crop&w=900&q=85' },
  { id: 3, name: 'Thabo Maseko', specialty: 'Modern grooming', bio: 'Thabo brings calm precision to modern cuts and the small finishing details people notice.', imageUrl: 'https://images.unsplash.com/photo-1703792686667-7486746389a1?auto=format&fit=crop&w=900&q=85' },
];
...
function businessDateInputValue(offsetDays = 0) {
  ...
  const businessDate = new Date(Date.UTC(Number(value.year), Number(value.month) - 1, Number(value.day) + offsetDays, 12));
  return businessDate.toISOString().slice(0, 10);
}

function barberImage(barber: Barber) {
  return stockBarbers.find((stockBarber) => stockBarber.name === barber.name)?.imageUrl ?? barber.imageUrl;
}
...
function Footer() {
  return <footer className="site-footer"><div className="container"><div className="footer-top"><div><div className="footer-mark">Good hair days<br /><em>start here.</em></div><p className="muted" style={{ maxWidth: 300, marginTop: 28 }}>A neighbourhood barber shop for considered cuts, close shaves and the people of Cape Town.</p></div><div className="footer-links"><div><div className="mono" style={{ color: 'hsl(var(--accent))', marginBottom: 20 }}>Explore</div><Link href="/services">Services</Link><Link href="/about">Our story</Link><Link href="/contact">Contact</Link><Link href="/booking">Book a chair</Link></div><div><div className="mono" style={{ color: 'hsl(var(--accent))', marginBottom: 20 }}>Visit</div><span className="footer-address">17 Bree Street<br />Cape Town, 8001<br />Mon–Fri · 09:00–18:00<br />Sat · 08:00–16:00<br />Sun · Closed</span><a href={SHOP_DIRECTIONS} target="_blank" rel="noreferrer" data-testid="link-footer-directions">Get directions</a></div><div><div className="mono" style={{ color: 'hsl(var(--accent))', marginBottom: 20 }}>Contact</div><a href={SHOP_PHONE_HREF}>{SHOP_PHONE}</a><a href={`mailto:${SHOP_EMAIL}`}>{SHOP_EMAIL}</a><a href={SHOP_WHATSAPP} target="_blank" rel="noreferrer" data-testid="link-footer-whatsapp">WhatsApp the shop</a></div></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} CROWN & BLADE</span><span>Independent barbering on Bree Street</span><span><Link href="/terms">Terms</Link> &nbsp;·&nbsp; <Link href="/privacy">Privacy</Link></span></div></div></footer>;
}
...
function Shell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (location !== '/' || sessionStorage.getItem('cb-offer-seen')) return undefined;
    const t = window.setTimeout(() => setOffer(true), 4000);
...
function Booking() {
  ...
  const [date, setDate] = useState(() => businessDateInputValue(1));
